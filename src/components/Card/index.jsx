function Card({
  ...
  function removeTransaction(event) {
    console.log(
      [...listTransactions].filter((a) => {
        console.log(listTransactions.indexOf(a));
        return listTransactions.indexOf(a) !== event.target.parentElement.id;
      })
    );
    ...
    setFilteredList(
      [...listTransactions].filter(
        (a) => listTransactions.indexOf(a) != event.target.parentElement.id
      )
    );
  }
  ...
  return (
    ...
    >
      ...
      <p>R$ {Math.abs(transaction.value)}, 00</p>
      <svg
      ...
      <h6>{transaction.type === "entrada" ? "Entrada" : "Despesa"}</h6>
    </li>
  );
}
